...
def preorden(tree: "Tree"):
    """Recorrido en preorden: Nodo, Izquierda, Derecha"""
    resultado = [tree.cargo]

    if tree.left:
        resultado.extend(preorden(tree.left))
    if tree.right:
        resultado.extend(preorden(tree.right))

    return resultado


def inorden(tree: "Tree"):
    """Recorrido en inorden: Izquierda, Nodo, Derecha"""
    resultado = []

    if tree.left:
        resultado.extend(inorden(tree.left))
    resultado.append(tree.cargo)
    if tree.right:
        resultado.extend(inorden(tree.right))

    return resultado


def postorden(tree: "Tree"):
    """Recorrido en postorden: Izquierda, Derecha, Nodo"""
    resultado = []

    if tree.left:
        resultado.extend(postorden(tree.left))
    if tree.right:
        resultado.extend(postorden(tree.right))
    resultado.append(tree.cargo)
